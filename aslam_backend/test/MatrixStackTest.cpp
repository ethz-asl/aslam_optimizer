...
TEST(MatrixStackTestSuites, testMatrixStack)
{
  try
  {
    ...
    // Push() and top() should return the same matrix again
    {
      ...
      const auto M = Eigen::Matrix<double, 2, 3>::Random().eval();
      stack.push(M);
    ...
    // Push() and top() should return the same matrix again
    {
      ...
      const auto M = Eigen::Matrix<double, 2, 4>::Random().eval();
      stack.push(M);
      EXPECT_EQ(M.size(), stack.numElements());
      EXPECT_EQ(1, stack.numMatrices());
    ...
    // Test that pop() yields intermediate matrices again
    {
      ...
      std::vector< Eigen::Matrix<double, 3, 3>, Eigen::aligned_allocator< Eigen::Matrix<double, 3, 3> > > storage;
      storage.push_back(Eigen::Matrix<double, 3, 3>::Identity());

      for (size_t i=0; i<10; ++i) {
        const auto M = Eigen::Matrix<double, 3, 3>::Random().eval();
        stack.push(M);
