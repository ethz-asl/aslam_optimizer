...
void exportGenericProjectionFunctions(T & proj)
{
  ...
  proj.def("isProjectionInvertible", &C::isProjectionInvertible,"Is the sensor model invertible? Ususally this is only true for a range/bearing sensor.");
  proj.def("createRandomKeypoint", &C::createRandomKeypoint, "Create a valid, random keypoint. This is useful for unit testing and experiments.");
  proj.def("createRandomVisiblePoint", &C::createRandomVisiblePoint, "Create a valid point in space visible by the camera.\np = createRandomVisiblePoint(depth).");
  proj.def("euclideanToKeypoint", &e2k<C>, "Map a 3x1 Euclidean point to a keypoint.\nk = euclideanToKeypoint(p)");
  proj.def("euclideanToKeypointJp", &e2kJp<C>,"Map a 3x1 Euclidean point to a keypoint and get the Jacobian of the mapping with respect to small changes in the point.\n(k, Jp) = euclideanToKeypoint(p)");
  proj.def("homogeneousToKeypoint", &eh2k<C>, "Map a 4x1 homogeneous Euclidean point to a keypoint.\nk = euclideanToKeypoint(p)");
  proj.def("homogeneousToKeypointJp", &eh2kJp<C>,"Map a 4x1 homogeneous Euclidean point to a keypoint and get the Jacobian of the mapping with respect to small changes in the point.\n(k, Jp) = homogeneousToKeypointJp(p)");
  proj.def("keypointToHomogeneous", &k2eh<C>, "Map a keypoint to a 4x1 homogeneous Euclidean point.\np = keypointToHomogeneous(k)");
  proj.def("keypointToHomogeneousJk", &eh2kJp<C>,"Map a keypoint to a 4x1 homogeneous Euclidean point and get the Jacobian of the mapping with respect to small changes in the keypoint.\n(p, Jk) = keypointToHomogeneousJk(k)");
  proj.def("keypointToEuclidean", &k2e<C>, "Map a keypoint to a 3x1 Euclidean point.\np = keypointToEuclidean(k)");
  proj.def("keypointToEuclideanJk", &e2kJp<C>,"Map a keypoint to a 3x1 Euclidean point and get the Jacobian of the mapping with respect to small changes in the keypoint.\n(p, Jk) = keypointToEuclideanJk(k)");
...
void exportGenericDistortionFunctions(T & dist)
{
  ...
  dist.def("undistort", &undistort2<C>, "Undistort a keypoint and get the Jacobian with respect to small chages in the input (distorted) keypoint.");
  dist.def("distortParameterJacobian", &distortParameterJacobian<C>, "Get the Jacobian of the distortion operation with respect to small changes in the distortion parameters");
...
void exportRadialTangentialDistortionFunctions()
{
  ...
  class_<RadialTangentialDistortion, boost::shared_ptr<RadialTangentialDistortion> > rtDistortion("RadialTangentialDistortion", init<>());
  exportGenericDistortionFunctions<RadialTangentialDistortion>(rtDistortion);

  rtDistortion.def(init<double, double, double, double>(("RadialTangentialDistortion(double k1, double k2, double p1, double p2)")));
...
void exportOmniProjection(std::string name)
{
  ...
  omniProjection.def(init<>( (name + "(distortion_t distortion)").c_str()))
	.def(init<double, double, double, double, double, int, int, D>( (name + "(double xi, double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int resolutionU, int resolutionV, distortion_t distortion)").c_str()))
	.def(init<double, double, double, double, double, int, int>( (name + "(double xi, double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int resolutionU, int resolutionV)").c_str()))
	...
	.def("distortion",&OmniProjection<D>::distortion, return_internal_reference<>())
...
void exportPinholeProjection(std::string name)
{
  ...
  pinholeProjection.def(init<>((name + "(distortion_t distortion)").c_str()))
	.def(init<double, double, double, double, int, int, D>((name + "(double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int resolutionU, int resolutionV, distortion_t distortion)").c_str()))
	.def(init<double, double, double, double, int, int>((name + "(double focalLengthU, double focalLengthV, double imageCenterU, double imageCenterV, int resolutionU, int resolutionV)").c_str()))
	/// \brief The horizontal focal length in pixels.
